...
/** Writes an .env.local file, replacing it wholesale so re-runs stay predictable. */
function writeEnvLocal(filePath: string, entries: Record<string, string>, note: string): void {
  const body = Object.entries(entries)
    .map(([key, value]) => `${key}=${value}`)
    .join("\n");
  const content = `# Generated by \`yarn passport:bootstrap\`. Safe to commit? No — regenerate instead.\n# ${note}\n\n${body}\n`;
...
async function main(): Promise<void> {
    ...
    if (!state.eventTransactionIds?.length) {
      ...
      const messages = [
        ...
        ...DEMO_EVENTS.map(demo =>
          buildEvent({ type: demo.type, serial: state.serial!, tokenId, actor, payload: demo.payload }),
        ),
      ];
  ...
  writeEnvLocal(
    path.join(REPO_ROOT, "packages", "indexer", ".env.local"),
    {
      ...
      INDEXER_TOPIC_IDS: state.topicId!,
    },
