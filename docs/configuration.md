...
| `BOOTSTRAP_NEW_PRODUCT` | Set to `true` for one run to register another demo product on the same registry and collection. Earlier products are kept in `passport.state.json` and stay in `INDEXER_TOPIC_IDS`. |
